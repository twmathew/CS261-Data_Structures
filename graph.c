...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	//Clear the visited property off all the nodes
	clearVisited(graph);

	//Check if they are neighbors.
	if (isAdjacent(source, destination) == 1)
		return 1;

	//Need to use provided stuff in Deque.c: initialize, and push values onto it as they are visited. Use it like a stack.
	struct Deque* myDeque = dequeNew();
	struct Vertex* curVert = source;
	dequePushFront(myDeque, source);

	source->isVisited = 1;

	//Check dequeIsEmpty each iteration. If it is true, we are finished. return 0 because no path exists.
	while (dequeIsEmpty(myDeque) == 0)
	{
		//Current = the top of the Stack. Check if it == destination
		curVert = dequeFront(myDeque);
		if (curVert == destination)
			return 1;
		//Pop curVert off the stack
		dequePopFront(myDeque);

		//Push all Neighbors of the curVert onto the stack
		for (int i = 0; i < curVert->numNeighbors; i++)
		{
			Vertex* neighbor = curVert->neighbors[i];
			//Has the current neighbor been visited?
			if (neighbor->isVisited == 0)
			{
				//If not, push it on the stack and mark it as visited
				dequePushFront(myDeque, neighbor);
				(curVert->neighbors[i])->isVisited = 1;
			}
		}
	
	}
	//If we never found the destination, return 0
    return 0;
}
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	while (dequeIsEmpty(myDeque) == 0)
	{
		//Current = the back of the Queue. Check if it == destination
		curVert = dequeBack(myDeque);
		if (curVert == destination)
			return 1;
		//Pop curVert off the stack
		dequePopBack(myDeque);
